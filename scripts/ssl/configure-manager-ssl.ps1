...
function ConfigureIIS([string] $pfxfile, $Secure_String_Pwd, $domainname, $thumbprint) {
    # Import the cert to the machine on IIS
    log "Importing the certificate into LocalMachine\My"
    Import-PfxCertificate -FilePath $pfxfile -CertStoreLocation Cert:\LocalMachine\My -Password $Secure_String_Pwd

    # Create an ssl binding in SSL and add cert to it
    log "Creating a new web binding for https"
    New-WebBinding -Name "Default Web Site" -Protocol "https" -Port 443 -SslFlags 1 -HostHeader $domainname
...
function ConfigureManagerServicesTransportSecurity() {
    ...
    @(
        ...
        if (!$securityNode){
            log "ERROR: <transport> not found, cannot configure transport security"
            continue
        }
...
if ([String]::IsNullOrEmpty($pfxfile) -and [String]::IsNullOrEmpty($domainname) -and [String]::IsNullOrEmpty($thumbprint)) {
...
if ([String]::IsNullOrEmpty($pfxfile) -or [String]::IsNullOrEmpty($domainname) -or [String]::IsNullOrEmpty($thumbprint)) {
  log "ERROR: All or one of pfxfile, domainname, or thumbprint is empty."
  exit 1
}

ConfigureIIS $pfxfile $Secure_String_Pwd $domainname $thumbprint
